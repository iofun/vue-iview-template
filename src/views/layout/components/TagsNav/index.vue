<template>
  <div class="tags-nav">
    ...
    >
      <li
        v-for="(item, key) of tagsMenuList"
        :key="key"
      ...
      >
        <transition-group name="taglist-moving-animation">
          <Tag
            ...
            :closable="item.name !== $config.pageName.home"
...
export default {
  ...
  methods: {
    ...
    handleTagsOption(type) {
      if (type.includes('all')) {
        ...
        const res = this.list.filter(
          item => item.name === this.$config.pageName.home
        );
        ...
        const res = this.list.filter(
          item =>
            routeEqual(this.currentRouteObj, item) ||
              item.name === this.$config.pageName.home
        );
    ...
    contextMenu(item, e) {
      if (item.name === this.$config.pageName.home) {
        return;
      }
      this.visible = true;
