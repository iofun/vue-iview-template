<template>
  <div
    :class="[
      prefixCls, 'tree-table',
      {'tree-table-border': border}
    ]"
  >
    <div
      v-if="showHeader"
      ref="header "
      :class="['ivu-tree-header',{'ivu-tree-border':bottomLine}]"
    >
      <slot name="header">
        <template v-for="(col, i) in tableColumns">
          <div :key="col._index" :class="[col.className]">
            <template>
              <Checkbox v-if="col._index === 0 && showCheckbox" :value="checkedAll" :indeterminate="indeterminate" @click.native.prevent="checkAll"></Checkbox>
              {{ col.title }}
              <span v-if="col.sortable" class="ivu-table-sort">
    ...
    <div class="ivu-tree-content" :style="bodyStyle">
      <tree-table-node
    ...
    <div v-if="!stateTree.length" :class="[prefixCls + '-empty']">{{ emptyText }}</div>
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    indeterminate() {
      ...
      const some = this.flatState.some(state => state.node.checked === true);
      if (some) {
        return true;
      }
      return false;
  ...
  methods: {
    transformCols() {
      ...
      const columnsWidth = {};
      this.tableColumns = [...this.columns].map((col, index) => {
        const column = Object.assign({ _index: index, _width: null }, col);
        if (!column.render && column.template) {
          const template = this.$scopedSlots[column.template];
          if (template) {
      ...
      this.tableWidth = this.tableColumns.map(cell => cell._width).reduce((a, b) => a + b, 0) + 1;
